...
queue_sizes=("-y 512") #(" " "8" "16" "64" "512" "1024") page fault queue size. Max supported size is 1024.
...
# Sets the memory limit for the cgroup 'xsbench_group' in MB
function set_memory_limit() {
	...
	# Ensure memory cgroup is mounted and xsbench_group exists
	if [ ! -d /sys/fs/cgroup/memory ]; then
		sudo mkdir -p /sys/fs/cgroup/memory
		sudo mount -t cgroup -o memory memory /sys/fs/cgroup/memory
	fi
	if [ ! -d /sys/fs/cgroup/memory/xsbench_group ]; then
		sudo cgcreate -g memory:xsbench_group
	fi
	# echo "$mem_limit_bytes" | sudo tee /sys/fs/cgroup/memory/xsbench_group/memory.limit_in_bytes >/dev/null
	sudo cgset -r memory.limit_in_bytes="$mem_limit_bytes" xsbench_group
...
function build_filename() {
    ...
    if [ -n "$quese_size" ]; then
      # Extract only the numeric value from $quese_size (e.g., "-y 512" -> "512")
      queue_value="${quese_size##* }"
      filename="$filename-$queue_value"
    else
      filename="$filename-no"
    fi
	else
...
function execute_benchmark() {
  ...
  local cmd_args="$args $queue_size"
  ...
  local command="cgexec -g memory:xsbench_group ../../$kernel $cmd_args"
...
function main() {
    ...
    echo "Creating cgroup 'xsbench_group' if it doesn't exist... ⏳"
    sudo cgcreate -g memory:xsbench_group
    adjust_the_settings
