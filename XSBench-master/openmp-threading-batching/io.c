...
Inputs read_CLI( int argc, char * argv[] )
{
    ...
    // pagefault queue size (-y)
    if( strcmp(arg, "-y") == 0 )
    {
      if( ++i < argc )
        input.pagefault_q_size = atoi(argv[i]);
      else
        print_CLI_error();
    }
    else
...
SimulationData binary_read( Inputs in )
{
  ...
  size_t nread = 0;
	...
	nread = fread(&SD, sizeof(SimulationData), 1, fp);
	...
	nread = fread(SD.num_nucs,       sizeof(int), SD.length_num_nucs, fp);
	...
	nread = fread(SD.index_grid, sizeof(int), SD.length_index_grid, fp);
