...
```bash
sudo ./run_vanilla.sh    # Baseline
sudo ./run_batching.sh   # Optimized versions with asynchronous fault handling
```

**What these scripts do**:
- Configure appropriate flags and inputs
- Limit memory to 95% of working set size using cgroups (simulates memory pressure)
...
1. Complete the GUPS experiments (Step 1 above)
2. Run the XSBench experiments (see `XSBench-master/` directory and consult its own `Artifact_Workflow.md`)
...
1. **Open the batching source file**: Navigate to the `src` directory and open `gups_single_threaded_batching.cc`. This is the version with asynchronous fault handling.
...
3. **Find the faulting PC in assembly**: 
   - Open the corresponding `.asm` file in the `src_assembly` directory (`gups_single_threaded_batching.asm`).
...
5. **Update the source code**:
   - Return to the source file (`gups_single_threaded_batching.cc`).
