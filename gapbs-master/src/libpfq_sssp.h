#ifndef LIBPFQ_SSSP_H_
#define LIBPFQ_SSSP_H_
...
const WGraph* graph_pointer = NULL;  // Global pointer to the graph. Required by the pagein_queue function
...
pvector<WeightT>* dist_pointer = NULL;       // store dist pointer to a global variable
WeightT* delta_pointer = NULL;            // store delta pointer to a global variable
vector<vector<NodeID> >* local_bins_pointers[NR_THREADS] = {}; // store local_bins pointer to a global variable 
...
struct state{
  ...
  WNode* neighborhood_itr;  // causes page fault
  // add your custom fields here
  WNode* neighborhood_itr_end; // neighborhood end
...
inline void resume_iteration(state &s){
  ...
  WNode* wn_itr_begin = s.neighborhood_itr;
  WNode* wn_itr_end = s.neighborhood_itr_end;
  pvector<WeightT> &dist = *(dist_pointer);
  WeightT delta = *(delta_pointer);
  // int tid = omp_get_thread_num();
  // vector<vector<NodeID> > &local_bins = *(local_bins_pointers[tid]);
  vector<vector<NodeID> > &local_bins = *(s.local_bins);

  for (auto wn_itr=wn_itr_begin; wn_itr!=wn_itr_end; ++wn_itr) {
    WNode wn = *wn_itr;   
...
inline void pagein_queue(vector<state> &queue){
  ...
  for (auto q_it = queue.begin(); q_it != queue.end(); ++q_it)
  {
    ...
    WNode *v_neighborhood_end = s.neighborhood_itr_end;
    WNode *faulty_address = s.neighborhood_itr;
