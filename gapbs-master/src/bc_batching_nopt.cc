...
void PBFS(const Graph &g, NodeID source, pvector<CountT> &path_counts,
  ...
  depths_pointer = &depths;       // store depths pointer to a global variable
  depths[source] = 0;
  ...
  global_g_out_start = g_out_start; // store g_out_start pointer to a global variable
  SETUP_GLOBAL_VARS();
    ...
    while (!queue.empty()) {
      ...
      for (auto q_iter = queue.begin(); q_iter < queue.end(); q_iter++) {
        ...
        for (NodeID* v_itr=g.out_neigh(u).begin(); v_itr != g.out_neigh(u).end(); ++v_itr) {
          NodeID v;
          TRY{
            v = *v_itr;   // PBFS trouble maker
          }CATCH{
            state s = {u, v_itr, 0, depth, &lqueue};
            ENQUEUE(s,resume_iteration_pbfs);
            break;  //go to the next vertex(u) since other edges/neighbors probably do not reside in memory
          }
          if ((depths[v] == -1) &&
          ...
          if (depths[v] == depth) {
            succ.set_bit_atomic(v_itr - g_out_start);
            #pragma omp atomic
            path_counts[v] += path_counts[u];
          }
        }
      }
      FLUSH_QUEUE(resume_iteration_pbfs);
      // }
      // else{
      //   #pragma omp for schedule(dynamic, 64) nowait
      //   for (auto q_iter = queue.begin(); q_iter < queue.end(); q_iter++) {
...
pvector<ScoreT> Brandes(const Graph &g, SourcePicker<Graph> &sp,
  ...
  // store global pointers to be used in pfq
  graph_pointer = &g;       
  scores_pointer = &scores; 
  path_counts_pointer = &path_counts;
  succ_pointer = &succ; 
  ...
  for (NodeID iter=0; iter < num_iters; iter++) {
    ...
    deltas_pointer = &deltas; // store deltas pointer to a global variable
    t.Start();
    bool break_flag = false;
    for (int d=depth_index.size()-2; d >= 0; d--) {
      // if(depth_index[d+1]-depth_index[d] > BATCHING_THRESHOLD){
      SETUP_GLOBAL_VARS();
      #pragma omp parallel for schedule(dynamic, 64) PFH_CLAUSES
      for (auto it = depth_index[d]; it < depth_index[d+1]; it++) {
        ...
        for (NodeID* v_itr=g.out_neigh(u).begin(); v_itr != g.out_neigh(u).end(); v_itr++) {
          if (succ.get_bit(v_itr - g_out_start)) {
            NodeID v;
            TRY {
              v = *v_itr;   // Brandes trouble maker
            } CATCH {
              state s = {u, v_itr, delta_u, 0, 0};
              ENQUEUE(s,resume_iteration_brandes);
              break_flag = true;  // skip current node(u)
              break;  //go to the next vertex(u) since other edges/neighbors probably do not reside in memory
            }
        ...
        {
          break_flag = false;
          continue; // do not update deltas and scores. jump to the next node (u)
        }
        deltas[u] = delta_u;
        scores[u] += delta_u;
      }
      FLUSH_QUEUES(resume_iteration_brandes);
      // }
      // else{
...
int main(int argc, char* argv[]) {
  ...
  if(cli.batching()){
    ...
    ips = (unsigned long *)malloc(2 * sizeof(unsigned long));
    ret_ips = (unsigned long *)malloc(2 * sizeof(unsigned long));
    ips[0] = 0x40883e;  // pbfs t maker
    ret_ips[0] = 0x3;
    ips[1] = 0x4080b2;  // brandes t maker
    ret_ips[1] = 0x3;
