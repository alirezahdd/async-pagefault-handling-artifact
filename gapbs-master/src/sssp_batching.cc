...
#include "libpfq_sssp.h"

inline
void RelaxEdges(const WGraph &g, NodeID u, WeightT delta,
                pvector<WeightT> &dist, vector <vector<NodeID>> &local_bins,
                bool batching, int status_idx, int omp_tid) {
  if(batching){
    WNode* neighborhood_itr_begin = g.out_neigh(u).begin();
    WNode* neighborhood_itr_end = g.out_neigh(u).end();
    for (WNode* wn_itr=neighborhood_itr_begin; wn_itr!=neighborhood_itr_end; ++wn_itr) {
      WNode wn;
      TRY{
        wn = *wn_itr;   // trouble maker
      }CATCH{
        //TODO
        state s = {u, wn_itr, neighborhood_itr_end, &local_bins};
...
pvector<WeightT> DeltaStep(const WGraph &g, NodeID source, WeightT delta,
  ...
  #pragma omp parallel PFH_CLAUSES
  {
    ...
    while (shared_indexes[iter&1] != kMaxBin) {
      ...
      if(curr_frontier_tail > BATCHING_THRESHOLD)
        batching = true;
      ...
      for (size_t i=0; i < curr_frontier_tail; i++) {
        ...
        if (dist[u] >= delta * static_cast<WeightT>(curr_bin_index))
          RelaxEdges(g, u, delta, dist, local_bins, batching, status_idx,omp_tid);
      }
      // clear queue (singular)
      if(batching) FLUSH_QUEUE(resume_iteration);
      ...
      while (curr_bin_index < local_bins.size() &&
        ...
        if(curr_bin_copy.size() > BATCHING_THRESHOLD/num_threads)
          batching = true;
        else
          batching = false;
...
int main(int argc, char* argv[]) {
  ...
  if(cli.batching()){
    ...
    ips[0] = 0x408dde;  // t maker
    ret_ips[0] = 0xb;
    ips[1] = 0x408fe5;  // t maker
    ret_ips[1] = 0xb;
