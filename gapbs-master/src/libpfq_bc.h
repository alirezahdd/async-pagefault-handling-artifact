...
pvector<NodeID>* depths_pointer = NULL;       // making the parent a global variable
const NodeID* global_g_out_start  = NULL;     // g_out_start
Bitmap* succ_pointer = NULL;                  // making the succ bitmap a global variable
pvector<CountT>* path_counts_pointer = NULL;  // making the path_counts a global variable
pvector<ScoreT>* scores_pointer = NULL;       // making the scores a global variable
pvector<ScoreT>* deltas_pointer = NULL;      // making the deltas a global variable
...
inline void resume_iteration_pbfs(state &s);
inline void resume_iteration_brandes(state &s);

inline void resume_iteration_pbfs(state &s){
  NodeID u = s.pivot_node;
  NodeID depth = s.depth;
  NodeID* v_iterator = s.neighborhood_itr;
  
  for (NodeID* v_itr = v_iterator; v_itr != (*graph_pointer).out_neigh(u).end(); ++v_itr) {
    NodeID v = *v_itr;
    if (((*depths_pointer)[v] == -1) &&
        (compare_and_swap((*depths_pointer)[v], static_cast<NodeID>(-1), depth))) {
      (*(s.lqueue)).push_back(v);
    }
    if ((*depths_pointer)[v] == depth) {
      (*succ_pointer).set_bit_atomic(v_itr - global_g_out_start);
      #pragma omp atomic
      (*path_counts_pointer)[v] += (*path_counts_pointer)[u];
    }
  }
}
...
inline void resume_iteration_brandes(state &s){
  ...
  ScoreT delta_u = s.delta_u;
  // NodeID depth = s.depth;
  const Graph &g = *(graph_pointer);
  // pvector<NodeID>& depths = *(depths_pointer);
  pvector<CountT>& path_counts = *(path_counts_pointer);
  Bitmap& succ = *(succ_pointer);
  const NodeID* g_out_start = global_g_out_start;
  pvector<ScoreT>& deltas = *(deltas_pointer);
  pvector<ScoreT>& scores = *(scores_pointer);
  
  for (NodeID* v_itr = v_iterator; v_itr != g.out_neigh(u).end(); ++v_itr) {
    NodeID v = *v_itr;
    if (succ.get_bit(v_itr - g_out_start)) {
