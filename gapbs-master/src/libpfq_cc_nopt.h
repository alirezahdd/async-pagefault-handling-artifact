#ifndef LIBPFQ_CC_NOPT_H_
#define LIBPFQ_CC_NOPT_H_
...
inline void readahead(NodeID u, NodeID* v_itr, bool* pre_faulted, const Graph &g) {
  if(!(*pre_faulted)) {
    NodeID* address_begin = v_itr;
    NodeID last_vertex_in_chunk = MIN(u+((131072-(u%131072))-1),g.num_nodes());
    NodeID* address_end = g.out_neigh(last_vertex_in_chunk).end();
    void* page_begin = (void*) address_begin;
    page_begin = (void *)(((long long)page_begin >> 12) << 12);
    size_t advise_length = (size_t)address_end - (size_t)page_begin;
    madvise(page_begin, advise_length, MADV_WILLNEED);    // prefaulting for the entire chunk of workload (1024 nodes)

    // struct iovec regions[MAX_IOVEC_SIZE];
    // int index = 0;
    // NodeID* address = v_itr;
    // NodeID last_vertex_in_chunk = MIN(u+((131072-(u%131072))-1),g.num_nodes());
    // void* page = (void*) address;
    // void* last_region_begin = (void *)(((long long)page >> 12) << 12);
    // void* last_region_end = (void *)((((long long)page >> 12)+1) << 12);
    // NodeID ra_u = u;
    // while(ra_u < last_vertex_in_chunk) {
    //   address = g.out_neigh(ra_u, neighbor_rounds).begin();
    //   void* page = (void*) address;
    //   page = (void *)(((long long)page >> 12) << 12);

    //   if(page < last_region_end) {
    //     ra_u++;
    //     continue;
    //   }
    //   if(page == last_region_end) {
    //     last_region_end = (void *)((((long long)page >> 12)+1) << 12);
    //     ra_u++;
    //     continue;
    //   }
    //   if(page > last_region_end ) {  //hole in the chunk
    //     regions[index].iov_base = last_region_begin;
    //     regions[index].iov_len = (size_t)last_region_end - (size_t)last_region_begin;
    //     index++;
    //     if(index >= MAX_IOVEC_SIZE) {
    //       process_madvise(pidfd, regions, index, MADV_WILLNEED, 0);
    //       index = 0;
    //     }
    //     last_region_begin = page;
    //     last_region_end = (void *)((((long long)page >> 12)+1) << 12);
    //     ra_u++;
    //     continue;
    //   }
    // }
    // madvise(last_region_begin, (size_t)last_region_end - (size_t)last_region_begin, MADV_WILLNEED);
    // if(index){
    //   process_madvise(pidfd, regions, index, MADV_WILLNEED, 0);
    // }
    *pre_faulted = true;
...
inline void readahead2(NodeID u, NodeID* v_itr, bool* pre_faulted, const Graph &g) {
  if(!(*pre_faulted)) {
    NodeID* address_begin = v_itr;
    NodeID last_vertex_in_chunk = MIN(u+((131072-(u%131072))-1),g.num_nodes());
    NodeID* address_end = g.out_neigh(last_vertex_in_chunk).end();
    ...
    page_begin = (void *)(((long long)page_begin >> 12) << 12);
    size_t advise_length = (size_t)address_end - (size_t)page_begin;
    madvise(page_begin, advise_length, MADV_WILLNEED);    

    // readahead for in-neighbors as well
    address_begin = g.in_neigh(u).begin();
    address_end = g.in_neigh(last_vertex_in_chunk).end();
    page_begin = (void*) address_begin;
    page_begin = (void *)(((long long)page_begin >> 12) << 12);
    advise_length = (size_t)address_end - (size_t)page_begin;
    madvise(page_begin, advise_length, MADV_WILLNEED);    
    *pre_faulted = true;
  }
}
...
// inline void readahead_p2(NodeID u, NodeID* v_itr, bool* pre_faulted, 
//                           const Graph &g, int neighbor_rounds, pvector<NodeID> &comp, NodeID c) {
//   if(!(*pre_faulted)) {            // skipping if we've already faulted for this chunk of workload
//     struct iovec regions[MAX_IOVEC_SIZE];
//     int index = 0;
//     NodeID* address_begin = v_itr;
//     //NodeID* address_end = g.out_neigh( MIN(u+((131072-(u%131072))-1),g.num_nodes())).end();
//     NodeID* address_end = g.out_neigh(u, neighbor_rounds).end();
//     NodeID last_vertex_in_chunk = MIN(u+((131072-(u%131072))-1),g.num_nodes());
...
//       if(comp[ra_u] == c)
//         continue;
//       address_begin = g.out_neigh(ra_u, neighbor_rounds).begin();
//       if(ra_u == u)   // only for the first iteration
//         address_begin = v_itr;
//       address_end = g.out_neigh(ra_u, neighbor_rounds).end();
//       void* page_begin = (void*) address_begin;
//       void* page_end = (void*) address_end;
//       page_begin = (void *)(((long long)page_begin >> 12) << 12);
//       page_end = (void *)((((long long)page_begin >> 12) +1)<< 12);

//       if(page_end<=last_page_end) // we have already advised this region
//         continue;
//       if(page_begin < last_page_end) // the begining of the region was already advised. skip advised part
//         page_begin = last_page_end;
...
// inline void readahead_p3(NodeID u, NodeID* v_itr, bool* pre_faulted, 
//                           const Graph &g, int neighbor_rounds, pvector<NodeID> &comp, NodeID c) {
//   if(!(*pre_faulted)) {            // skipping if we've already faulted for this chunk of workload
//     struct iovec regions[MAX_IOVEC_SIZE];
//     int index = 0;
//     NodeID* address_begin = v_itr;
//     NodeID* address_end = g.out_neigh(u, neighbor_rounds).end();
//     NodeID last_vertex_in_chunk = MIN(u+((CHUNK_SIZE-(u%CHUNK_SIZE))-1),g.num_nodes());
//     void* last_page_end = (void*) 0;
...
//     index = 0;
//     last_page_end = (void*) 0;
//     //readahead for in-neighbors TBD
//     for (NodeID ra_u = u; ra_u < last_vertex_in_chunk; ra_u++){
//       if(comp[ra_u] == c)
//         continue;
//       address_begin = g.in_neigh(ra_u).begin();
//       address_end = g.in_neigh(ra_u).end();
//       void* page_begin = (void*) address_begin;
//       void* page_end = (void*) address_end;
//       page_begin = (void *)(((long long)page_begin >> 12) << 12);
//       page_end = (void *)((((long long)page_begin >> 12) +1)<< 12);
