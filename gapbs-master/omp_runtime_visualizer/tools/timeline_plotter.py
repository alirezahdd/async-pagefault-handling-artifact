...
class TimelinePlotter:
    ...
    def create_timeline_plot(self, output_file='thread_timeline.pdf'):
    ...
    def write_statistics_file(self):
        """Write overall statistics to a file (omp_threads.stats)"""
        total_times = defaultdict(float)
        ...
        overall_total = sum(total_times.values())
        if overall_total > 0:
            with open('omp_threads.stats', 'w') as stats_file:
                field_width = 20
                stats_file.write(f"{'Active:':<{field_width}} {total_times['active']:10.2f}ms {total_times['active']/overall_total*100:8.2f}%\n")
                stats_file.write(f"{'Idle-Barrier:':<{field_width}} {total_times['idle_barrier']:10.2f}ms {total_times['idle_barrier']/overall_total*100:8.2f}%\n")
                stats_file.write(f"{'Idle-Sequential:':<{field_width}} {total_times['idle_sequential']:10.2f}ms {total_times['idle_sequential']/overall_total*100:8.2f}%\n")
                stats_file.write(f"{'Total:':<{field_width}} {total_times['idle_sequential'] + total_times['idle_barrier'] + total_times['active']:10.2f}ms\n")

def main():
    parser = argparse.ArgumentParser(description='Create OpenMP thread timeline visualization')
    parser.add_argument('parser_output_file', help='Path to the parser output file')
    parser.add_argument('output_plot_file', nargs='?', default='thread_timeline.pdf', 
                       help='Output plot filename (default: thread_timeline.pdf)')
    parser.add_argument('-s', '--stats', action='store_true', 
                       help='Write statistics to omp_threads.stats file')
    
    args = parser.parse_args()
    
    try:
        plotter = TimelinePlotter(args.parser_output_file)
        plotter.parse_output_file()
        plotter.analyze_thread_states()
        plotter.create_timeline_plot(output_file=args.output_plot_file)
        
        # Write statistics file if requested
        if args.stats:
            plotter.write_statistics_file()
            print("Statistics written to omp_threads.stats")
        
    except FileNotFoundError:
        print(f"Error: File '{args.parser_output_file}' not found.")
        sys.exit(1)
