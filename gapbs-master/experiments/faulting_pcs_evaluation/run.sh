...
kernels=("bc" "bfs" "cc" "pr" "sssp" "tc") #("bc" "bfs" "cc" "pr" "sssp" "tc") #("bc_batching" "bfs_batching" "cc_batching" "pr_batching" "sssp_batching" "tc_batching")
graphs=("road" "urand")   #("road" "kron" "twitter" "urand" "web")

available_memory=("30") #("100" "90" "80" "70" "60" "50" "40" "30")

ocfs=("1") #("1" "2" "4" "8" "16" "32" "64" "128")
batching=("")   #("" "-q")         batch pagefaults(-q) or not
...
file_partition="sda1" #might differ in your system. use lsblk to figure it out
#file_partition="nvme0n1p2"  #might be nvme1n1p2. use lsblk to figure it out

declare -A kernel_args
kernel_args["bc"]="-i3 -n1"       		# Default "-i4 -n16"
kernel_args["bfs"]="-n1"          		# Default "-n64"
kernel_args["cc"]="-n1"           		# Pact "-n5" Default "-n16"
kernel_args["pr"]="-i1000 -t1e-4 -n1" # Default "-i1000 -t1e-4 -n16"
kernel_args["sssp"]="-n1 -d2"     		# Default "-n64 -d2"
kernel_args["tc"]="-n1"      					# Default "-n3 -p"		# p is defined for partial exe
kernel_args["tc_road"]="-n1" 					# Default "-n3 -p"		# p is defined for partial exe
...
function execute_benchmark() {
  ...
  local command="perf trace --pf=maj -o ${stat_dirs[trace]}/$output_file_name.trace cgexec -g memory:gapbs_group ../../$kernel -f $graph_path $cmd_args"
